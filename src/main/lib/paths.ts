...
export function migrateXdgPaths(): void {
  ...
  migrateIfNeeded(path.join(oldBase, 'download-cache'), path.join(cacheDir(), 'download-cache'))
  ...
  migrateIfNeeded(path.join(oldBase, 'port-locks'), path.join(stateDir(), 'port-locks'))
