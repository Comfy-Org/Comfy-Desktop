...
export function registerInstallationHandlers(): void {
  ...
  ipcMain.handle('install-instance', async (_event, installationId: string, express?: boolean) => {
    ...
    if (source.install) {
      ...
      // earliest local-install dispatch on the person profile, so the funnel can
      // tell whether a user who onboarded ever actually started an install —
      // regardless of the session it happened in.
