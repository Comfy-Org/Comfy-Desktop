...
export const DATADOG_MIRRORED_EVENT_NAMES: ReadonlySet<string> = new Set([
  ...
  // Assets scan-pipeline failures: the eleven failure names in assetsTap's
  ...
  // scanner.hash_discarded_modified, scanner.invalid_mtime) stay PostHog-only
  // — the same split as every other group above. invalid_mtime counts files
  // skipped for a pre-epoch mtime: a property of the user's files, not a
  // scanner fault a release could regress.
